...
getDemandeNontraiteSelonEmployeeConnected:function(req,res) { 
    ...
    getListDemandeInterventionNonTraiter:function (req,res) {
        Demandeintervention.find({or:[{status :  'initiale' },{status : 'preventive' },{status :  'valider' },{status :  'en cours' },{status :  'reinitialiser' }]}).populateAll().exec(function (err, demandeintervention) {
            if(err){
